...
Let's load what we need to build a model" the configuration and the covariate data.  Notice that we select the covariates that are included in our configuration plus "class".


```{r load_data}
cfg = read_configuration(scientificname = "Mola mola", version = "v1")
model_input = read_model_input(scientificname = "Mola mola", version = "v1") |>
...
rec
```
We also need to handle factored covariates, like `month` might factored.  We do so by building in an automated conversion to numeric values.  Oddly, the step function is called `step_dummy()` which simply transforms the month column into a series of columns `Jan`, `Feb`,..., `Dec` where each row's value is 1 for the row's month and 0 for all the other months.

```{r step_dummy_month}
if ("month" %in% cfg$keep_vars){
  rec = rec |>
    step_dummy(month)
}
rec
...
For each model type there is a convenient species distribution model specification (`sdm_spec_*`) version designed to handle spatial data. It is here that we specify the hyperparameters to tune.  In general we accept the defaults, but for `random_forest` we ask that we include tuning on the number of trees, too.
...
wflow = workflow_set(
  ...
  models = list(
      glm = sdm_spec_glm(),
      rf = sdm_spec_rf(trees = tune()), #<-- tune sdm defaults plus trees
...
wflow <- wflow |>
  workflow_map("tune_grid",
    ...
    verbose = TRUE)
```
Here's a quick plot of the outputs, with each metruic being showcased.
```{r plot_wflow}
autoplot(wflow)
```
Note that "default_glm" only has one result rather than three.  That is because there are no hyperparameters to tune in general linearized models.
...
```{r pd_plot}
model_fit_pdp(model_fits, id = "default_glm")
```
